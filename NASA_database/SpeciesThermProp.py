...
def SpeciesThermProp(self, species, T, MassorMolar, echo):
    ...
    Delta_n = 1. - swtCondensed - np.dot(Delta_n_per_mole, Element_matrix[1,:]) 
    ...
    if ctTInt > 0:
        ...
        if (T < tRange[0][0]) or (T > tRange[ctTInt-1][1]) and echo:
            print('T out of range [%.2f - %.2f] [K] for %s' % (tRange[0][0], tRange[ctTInt][1], FullName))
            Cp0 = []
            Cv0 = []
            H0 = []
            Ef0 = []
        ...
        H0 = R0 * T * (sum(a[tInterval] * T**np.array(tExponents[tInterval]) * aux) + b[tInterval][0] / T)
        Ef0 = Hf0 - Delta_n * R0 * Tref
        E0 = Ef0 + (H0 - Hf0) - (1- swtCondensed) * R0 * (T - Tref)
        aux = np.array([-1/2, -1, np.log(T), 1, 1/2, 1/3, 1/4, 0])
        S0 = R0 * (sum(a[tInterval] * T**np.array(tExponents[tInterval]) * aux) + b[tInterval][1])
