...
def equilibrium(self, N_CC, phi, pP, TP, vP):
    ...
    while DeltaNP > C.tolN and it < itMax:
        it += 1
        # Gibbs free energy
        G0RT =  -(g0[temp_ind_nswt] / R0TP + log(N0[temp_ind_nswt, 0] / NP) + log(pP))
        # Construction of matrix A
        A21 = np.concatenate((N0[temp_ind, 0] * A0_T[:, temp_ind], [N0[temp_ind, 0]]))
        A22[-1, -1] = -NP
        ...
        if BRATIO < 1e-5:
            ...
            SIZE = -log(C.tolN) 
        for n, n_log_new in zip(N0[:, 0], x[0:temp_NS + 1]):
