...
def temp_values(S, NatomE, tol):
    ...
    temp_NS = S.NS 
    temp_ind_remove = []
    return (temp_ind, temp_ind_nswt, temp_ind_swt, temp_ind_E, temp_NE, temp_NG, temp_NS, temp_ind_remove)
...
def update_temp(temp_ind, temp_NG, temp_NS, N0, zip1, zip2, ls1, ls2, NP, SIZE):
    """ Update temp items """
    ...
    return (temp_ind, temp_ind_swt, temp_ind_nswt, temp_NG, temp_NS)

def update_matrix_A1(A0, temp_NG, temp_NS, temp_ind, temp_ind_E):
    """ Update stoichiometric submatrix A1 """
    A11 = np.eye(temp_NS)
    A11[temp_NG::, temp_NG::] = 0. # For condensed species
...
def update_matrix_A2(A0_T, A21, A22, N0, NP, temp_ind, temp_NG, temp_ind_E, temp_NS):
    """ Update stoichiometric submatrix A2 """
    A21 = np.concatenate((A0_T[np.ix_(temp_ind_E, temp_ind)], [np.zeros(temp_NS)]))
    A21[0:-1, 0:temp_NG] = N0[0:temp_NG, 0] * A21[0:-1, 0:temp_NG]
    A21[-1, 0:temp_NG] = N0[0:temp_NG, 0]
    A22[-1, -1] = -NP
    return np.concatenate((A21, A22), axis=1)

def update_matrix_A(A0_T, A1, A21, A22, N0, NP, temp_ind, temp_ind_nswt, temp_NG, temp_ind_E, temp_NS):
    """ Update stoichiometric matrix A """
    A2 = update_matrix_A2(A0_T,  A21, A22, N0, NP, temp_ind_nswt, temp_NG, temp_ind_E, temp_NS)
...
def apply_antilog(N0, N0_log, NP_log, temp_NG, temp_ind_nswt):
    N0[temp_ind_nswt, :] = np.concatenate((exp(N0_log).reshape(temp_NG, 1), N0[temp_ind_nswt, 1].reshape(temp_NG, 1)), axis=1)
    NP = exp(NP_log)
    return (N0, NP)

def compute_STOP(NP_0, NP, DeltaNP, zip1, zip2, zip3, zip4, temp_ind_swt):
    DeltaN1 = max(np.array([n * abs(n_log) / NP for n, n_log in zip(zip1, zip2)]))
    # if temp_ind_swt:
    #     DeltaN2 = max(np.array([abs(n_log) / NP for n, n_log in zip(zip3, zip4)]))
    # else:
    #     DeltaN2 = 0.
...
def equilibrium(self, N_CC, phi, pP, TP, vP):
    ...
    (temp_ind, temp_ind_nswt, temp_ind_swt, temp_ind_E,
     temp_NE, temp_NG, temp_NS, temp_ind_remove) = temp_values(S, NatomE, self.C.tolN)
    # Update temp values
    temp_ind, temp_ind_swt, temp_ind_nswt, temp_NG, temp_NS = update_temp(temp_ind, temp_NG, temp_NS, N0, N0[ind_A0_E0, 0], ind_A0_E0, temp_ind_swt, temp_ind_nswt, NP=self.C.tolN, SIZE=SIZE)
    ...
    while STOP > C.tolN and it < itMax:
        ...
        A = update_matrix_A(A0_T, A1, A21, A22, N0, NP, temp_ind, temp_ind_nswt, temp_NG, temp_ind_E, temp_NS)
        ...
        e = relax_factor(NP, N0[temp_ind_nswt, 0], x[0:temp_NG], x[-1], SIZE)  
        # Apply correction
        N0_log = log(N0[temp_ind_nswt, 0]) + e * x[0:temp_NG]
        N0[temp_ind_swt, 0] = N0[temp_ind_swt, 0] + e * x[temp_NG:0:temp_NS]
        NP_log = log(NP) + e * x[-1]
        # Apply antilog
        N0, NP = apply_antilog(N0, N0_log, NP_log, temp_NG, temp_ind_nswt)
        # print(pd.DataFrame(N0))
        # Update temp values in order to remove species with moles < tolerance
        temp_ind, temp_ind_swt, temp_ind_nswt, temp_NG, temp_NS = update_temp(temp_ind, temp_NG, temp_NS, N0, N0[temp_ind, 0], temp_ind, temp_ind_swt, temp_ind_nswt, NP=NP, SIZE=SIZE)
        # Update matrix A
        A1 = update_matrix_A1(A0, temp_NG, temp_NS, temp_ind, temp_ind_E)
        # Print moles per iteration
        # print_moles(N0[:, 0], S.LS, it)
        # Compute STOP criteria
        STOP = compute_STOP(NP_0, NP, x[-1], N0[temp_ind, 0], x[0:temp_NS], N0[temp_ind_swt, 0], x[temp_NG:temp_NS], temp_ind_swt)
