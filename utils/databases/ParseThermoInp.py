...
def ParseThermoInp(reducedDB):
    filepath = 'databases/DB_master.pkl'
    if not existFile(filepath):
        fid = open('databases/thermo.inp', 'r')
        if reducedDB:
        ...
        while True:
            ...
            aux = DB_master(tline, fid)
            struct.update({aux.name: aux})

        if reducedDB:
            struct = DB_master_reduced(struct)
            # Save DB master reduced
            f = open("databases/DB_master_reduced.pkl", "wb")
            pickle.dump(struct, f)
            f.close()
        else:
            # Save DB master
            f = open("databases/DB_master.pkl", "wb")
            pickle.dump(struct, f)
            f.close()

        print('OK!')

    else:  # Load DB master reduced
...
class DB_master():
    def __init__(self, tline, fid):
        self.read_line(tline, fid)

    def read_line(self, tline, fid):
